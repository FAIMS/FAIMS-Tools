...
while($line = <>) {
    ...
    if($line =~ "ref=\"([^\"]+)\"") {
        ...
        if($line =~ "<select[^1]" && $line !~ "type=\"camera\"" && $line =~ "faims_attribute_name=\"([^\"]+)\"") {
            push @pops, "populateCheckBoxGroup(\"$path\", makeVocab(\"$1\"));";
        } elsif($line =~ "<select1" && $line =~ "faims_attribute_name=\"([A-Za-z0-9 \-]+)\"") {
            $attr_name = $1;
            if($line =~ "appearance=\"full\"") {
                push @pops, "populateRadioGroup(\"$path\", makeVocab(\"$attr_name\"));";
            } elsif($line =~ "appearance=\"compact\"") {
                push @pops, "populateList(\"$path\", makeVocab(\"$attr_name\"));";
            } else {
                push @pops, "populateDropDown(\"$path\", makeVocab(\"$attr_name\"));";
            }    
